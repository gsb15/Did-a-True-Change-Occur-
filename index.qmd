...
Table: Key Analytical Definitions {#tbl-definitions}
...
| **Bayesian Posterior Standard Deviation** | \\[ \sqrt{\frac{r \times \text{population variance}}{1}} \\]  \\ where *r* = prior ICC and population variance is the prior TEM variance. | A practitioner observes a countermovement jump power test difference of -418. With prior literature showing an ICC of 0.834, Bayesian updating yields a posterior standard deviation of 90.6. |
...
To demonstrate Bayesian updating within a sport science context, we simulate weekly testing of peak concentric power for an athlete who performs countermovement jump testing twice per week [@Heishman2020]. The athlete completes three countermovement jumps with arm swing during each session, and the average peak concentric power across the three attempts is used for analysis. To better reflect real-world conditions, missing data were introduced for two testing periods. Parameters used to generate the simulation are reported in @tbl-simulation. A visualization of the simulated testing data is provided in @fig-serial-power.
